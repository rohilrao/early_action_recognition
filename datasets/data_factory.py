...
from datasets.smth_dataset import SomethingSomethingDataset
...
class DataFactory:
    ...
    def get_data_loader(config: Config, split_type='train'):
        ...
        elif config.dataset_name == 'SMTH':
            if split_type == 'train':
                dataset = SomethingSomethingDataset(config, split_type, train_img_tfms)
                return DataLoader(dataset, batch_size=config.batch_size,
                            shuffle=True, num_workers=config.num_workers)
            else:
                dataset = SomethingSomethingDataset(config, split_type, test_img_tfms)
                return DataLoader(dataset, batch_size=config.batch_size,
                            shuffle=False, num_workers=config.num_workers)
